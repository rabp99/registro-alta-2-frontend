...
/**
 * @ngdoc function
 * @name registroAltaFrontendApp.controller:SolicitudesDevolverCtrl
 * @description
 * # SolicitudesDevolverCtrl
 * Controller of the registroAltaFrontendApp
 */
angular.module('registroAltaFrontendApp')
.controller('SolicitudesDevolverCtrl', function ($scope, $utilsViewService, reutilizablesService, tiposService) {
    $scope.init = function() {
        $scope.tipos = [];
        $scope.reutilizables = [];
    ...
    var getReutilizables = function() {
        $scope.reutilizables = [];
        reutilizablesService.findEnLavanderia(function(data) {
    ...
    $scope.registrarDevolucion = function(reutilizables_selected) {       
        if (reutilizables_selected.length === 0) {
            Materialize.toast('No ha seleccionado ninguna indumentaria para registrar su devolución', 4000);
            return;
        }
        
        if (!confirm('¿Está seguro de registrar su devolución?')) {
            return;
        }
        ...
        reutilizablesService.registrarDevolucion({
